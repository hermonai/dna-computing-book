...
NAME = ("dna-computing" if DNA else "evolutor") + "-ch14-review"
BUILD = ROOT / "build/ch14"


def derived():
    spec = importlib.util.spec_from_file_location(
        "chapter14_reference", HERE / "reference.py"
    )
    ...
    table(
        "gate-table",
        ["a", "b", "c", "Copies"],
        [[v["a"], v["b"], v["c"], v["copies"]] for v in r["gate"]],
        "rrrr",
    )
    table(
        "reuse-table",
        ["Operations", "p = 0.99", "p = 0.999"],
        [
            [v["operations"], f"{v['p99']:.6f}", f"{v['p999']:.6f}"]
            for v in r["reuse"]
        ],
        "rrr",
    )
    plot("reuse-plot", r["reuse_curve"], "operations", ["p99", "p999"])
...
def main():
    ...
    if a.check or a.assets_only:
        print(
            "Chapter 14 computed artifacts verified"
            if a.check
            else "Chapter 14 computed artifacts generated"
        )
        return
