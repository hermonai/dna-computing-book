...
NAME = ("dna-computing" if DNA else "evolutor") + "-ch13-review"
BUILD = ROOT / "build/ch13"


def derived():
    spec = importlib.util.spec_from_file_location(
        "chapter13_reference", HERE / "reference.py"
    )
    ...
    table(
        "trace-table",
        ["Clause", "Species", "Copies"],
        [[v["stage"], v["species"], v["copies"]] for v in r["trace"]],
        "rrr",
    )
    table(
        "truth-table",
        ["Assignment", "Clause 1", "Clause 2", "Clause 3"],
        [[v["assignment"], *v["literals"]] for v in r["truth"]],
        "lrrr",
    )
    table(
        "coverage-table",
        ["Samples", "Witness probability"],
        [[v["samples"], f"{v['probability']:.6f}"] for v in r["coverage"]],
        "rr",
    )
    plot("coverage-plot", r["coverage_curve"], "samples", ["probability"])
...
def main():
    ...
    if a.check or a.assets_only:
        print(
            "Chapter 13 computed artifacts verified"
            if a.check
            else "Chapter 13 computed artifacts generated"
        )
        return
