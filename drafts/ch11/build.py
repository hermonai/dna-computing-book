...
NAME = ("dna-computing" if DNA else "evolutor") + "-ch11-review"
BUILD = ROOT / "build/ch11"


def derived():
    spec = importlib.util.spec_from_file_location(
        "chapter11_reference", HERE / "reference.py")
    ...
    table("code-table", ["Vertex", "Word (5' to 3')"], sorted(r["code"].items()), "ll")
    table("edge-table", ["Edge", "Word (5' to 3')"], r["edges"], "ll")
    table("selected-table", ["Accepted by the stated screens"],
          [[s] for s in (r["selected"] or ["No feasible subset"])], "l")
...
def main():
    ...
    if a.check or a.assets_only:
        print(
            "Chapter 11 computed artifacts verified"
            if a.check
            else "Chapter 11 computed artifacts generated"
        )
        return
