...
NAME = ("dna-computing" if DNA else "evolutor") + "-ch15-review"
BUILD = ROOT / "build/ch15"


def derived():
    spec = importlib.util.spec_from_file_location(
        "chapter15_reference", HERE / "reference.py"
    )
    ...
    table(
        "growth",
        ["Depth", "Distinct words"],
        [[v["depth"], v["words"]] for v in r["growth"]],
        "rr",
    )
    plot("growth-plot", r["growth"], "depth", ["words"])
...
def main():
    ...
    if a.check or a.assets_only:
        print(
            "Chapter 15 computed artifacts verified"
            if a.check
            else "Chapter 15 computed artifacts generated"
        )
        return
