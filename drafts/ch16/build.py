...
NAME = ("dna-computing" if DNA else "evolutor") + "-ch16-review"
BUILD = ROOT / "build/ch16"


def derived():
    spec = importlib.util.spec_from_file_location(
        "chapter16_reference", HERE / "reference.py"
    )
    ...
    table(
        "recognition",
        ["Prefix", "State"],
        [[v["prefix"], v["state"]] for v in r["recognition"]],
        "lr",
    )
    table(
        "addition",
        ["Position", "$x_i$", "$y_i$", "$c_i$", "$z_i$", "$c_{i+1}$"],
        r["addition"],
        "rrrrrr",
    )
    plot("noise-plot", r["noise"], "length", ["accept", "lost"])
...
def main():
    ...
    if a.check or a.assets_only:
        print(
            "Chapter 16 computed artifacts verified"
            if a.check
            else "Chapter 16 computed artifacts generated"
        )
        return
