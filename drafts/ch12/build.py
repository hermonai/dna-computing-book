...
NAME = ("dna-computing" if DNA else "evolutor") + "-ch12-review"
BUILD = ROOT / "build/ch12"


def derived():
    spec = importlib.util.spec_from_file_location(
        "chapter12_reference", HERE / "reference.py")
    ...
    table("trace-table", ["Stage", "Species", "Copies"],
          [[v["stage"],v["species"],v["copies"]] for v in r["trace"]], "lrr")
    table("noise-table", ["Stage", "True", "False", "True fraction"],
          [[v["stage"],f'{v["true"]:.2f}',f'{v["false"]:.2f}',f'{v["purity"]:.4f}'] for v in r["noisy"]], "rrrr")
    table("survival-table", ["Initial copies", "Detection probability"],
          [[v["copies"],f'{v["probability"]:.6f}'] for v in r["survival"]], "rr")
    plot("survival-plot",r["survival"],"copies",["probability"])
...
def main():
    ...
    if a.check or a.assets_only:
        print(
            "Chapter 12 computed artifacts verified"
            if a.check
            else "Chapter 12 computed artifacts generated"
        )
        return
