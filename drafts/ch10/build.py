...
NAME = ("dna-computing" if DNA else "evolutor") + "-ch10-review"
BUILD = ROOT / "build/ch10"


def derived():
    spec = importlib.util.spec_from_file_location(
        "chapter10_reference", HERE / "reference.py")
    ...
    if DNA:
        table("ladder-table", ["Length (bp)", "Distance (mm)"],
              [[v["length"],f'{v["distance"]:.3f}'] for v in r["ladder"]], "rr")
        table("uncertainty-table", [r"$\delta$", r"SD of $\widehat n_1$"],
              [[v["delta"],f'{v["sd"]:.4f}'] for v in r["uncertainty"]], "rr")
        table("window-table", ["Half-width (mm)", "Recovery", "Molecular purity"],
              [[v["width"],f'{v["recovery"]:.4f}',f'{v["purity"]:.4f}']
               for v in r["windows"]], "rrr")
        plot("profile-plot", r["profiles"], "x", ["target","other","total"])
        plot("window-plot", r["windows"], "width", ["recovery","purity"])
        selected = ["calibrate","infer_length","band_kernel","response_matrix",
                    "observe","unmix","roi_summary","collect_window"]
    else:
        table("trace-table", ["Token", "Expert", "Gate", "Admitted"],
              [[v["token"],v["expert"],v["gate"],"yes" if v["admitted"] else "no"]
               for v in r["trace"]], "rrrr")
        table("capacity-table", ["Capacity", "Admitted", "Dropped", "Unused slots"],
              [[v[k] for k in ("capacity","admitted","dropped","unused")]
               for v in r["sweep"]], "rrrr")
        plot("capacity-plot", r["sweep"], "capacity", ["admitted","dropped","unused"])
        selected = ["plan","expert","dispatch","dense_oracle","balance_loss",
                    "sparse_block","cost"]
...
def main():
    ...
    if a.check or a.assets_only:
        print(
            "Chapter 10 computed artifacts verified"
            if a.check
            else "Chapter 10 computed artifacts generated"
        )
        return
